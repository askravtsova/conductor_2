    ...
    for root, dirs, files in os.walk(processed_data_dir):
        for npy_file in files:
            if npy_file.endswith('.npy'):
                ...
                # Check if landmarks have the expected shape (sequence_length, landmarks, 3)
                if landmarks.ndim != 3 or landmarks.shape[1:] != (21, 3):
                    print(f"Skipping {npy_file} due to unexpected shape {landmarks.shape}")
                    continue
                ...
                print(f"After padding, {npy_file} has shape: {landmarks.shape}")
...
# Saving the history to a CSV file
hist_df = pd.DataFrame(history.history)
hist_csv_file = 'model_trained_data_14_Oct_bigger_dataset.csv'
hist_df.to_csv(hist_csv_file, index=False)

# Additional debugging information can be printed as needed
