    ...
    for root, dirs, files in os.walk(processed_data_dir):
        for npy_file in files:
            if npy_file.endswith('.npy'):
                ...
                # Map label to corresponding class
                if label == "2_4_60bpm":
...
# Build the LSTM model# Assuming 63 features for landmarks, velocity, and acceleration each
input_feature_size = 63 * 3  # Adjust this based on your actual number of features

model = Sequential()
model.add(LSTM(128, input_shape=(X_train.shape[1], input_feature_size), return_sequences=False))
...
with open('history_train_model_02_TESTDELETE.json', 'w') as f:
...
# or save to csv: 
hist_csv_file = 'Model_vel_accel_history_TESTDELETE.csv'
